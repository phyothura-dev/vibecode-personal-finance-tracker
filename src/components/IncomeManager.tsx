...
export default function IncomeManager({
  ...
  // Filter categories to only income categories
  ...
  const renderSortIcon = (field: IncomeSortField) => {
    if (sortField !== field) {
  ...
  return (
    <div className="space-y-6">
          ...
          <>
                ...
                <tfoot className="bg-slate-50 border-t-2 border-slate-200 font-semibold text-slate-800 text-sm">
                  <tr>
                    <td colSpan={3} className="py-3.5 px-6">
                      စုစုပေါင်း ဝင်ငွေ ({incomes.length} ခု)
                    </td>
                    <td className="py-3.5 px-4 text-right text-[#059669] font-bold text-base">
                      +{currencySymbol}{totalIncomeAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </td>
            ...
            <div className="md:hidden divide-y divide-slate-100">
              ...
              <div className="p-4 bg-slate-50 border-t-2 border-slate-200 flex items-center justify-between font-semibold text-sm">
                <span className="text-slate-700">စုစုပေါင်း ဝင်ငွေ ({incomes.length} ခု)</span>
                <span className="text-[#059669] text-base font-bold">
                  +{currencySymbol}{totalIncomeAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
              </div>
            </div>
      ...
      {/* Confirmation Dialog */}
      {deletingId && (
        <div id="delete-income-confirm-modal" className="fixed inset-0 z-50 flex items-center justify-center p-4">
